...
namespace PowerPlatform.SpecflowExtensions.Commands
{
    public class AssertFormStateCommand : BrowserOnlyCommand
    {
        ...
        public AssertFormStateCommand(IObjectContainer container, EntityReference crmRecord, Table visibilityCriteria) :
            base(container)
        {
        ...
        {
            var form = GlobalContext.ConnectionManager.GetCurrentBrowserSession(_seleniumContext)
                .GetApp<CustomerEngagementApp>(_container).OpenRecord(new OpenFormOptions(_crmRecord));
        }
